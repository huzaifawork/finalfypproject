...
class _TableReservationPageState extends State<TableReservationPage>
    ...
    try {
      ...
      if (isActuallyPersonalized) {
        ...
        final aiRecommendations =
          ...
          // The API returns table data in 'tableId' field, not 'table'
          final table = item['tableId'] ?? item['table'] ?? item;
          return {
            '_id': table['_id'] ?? '',
  ...
  Future<void> _loadAllTables() async {
    try {
      ...
      print('🍽️ Loading all tables...');

      // First try to get all tables from the main tables API
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data.isNotEmpty) {
            print('✅ Got ${data.length} tables from main API');
            setState(() {
              _allTables = data
        ...
        print('❌ Error loading from main tables API: $e');
      }

      // Fallback to popular tables API if main API fails
      print('📈 Falling back to popular tables API...');
      final response = await RecommendationService.getPopularTables(limit: 20);

      if (response['success'] == true) {
        final tables = response['popularTables'] ?? [];
        print('✅ Got ${tables.length} tables from popular API');
        setState(() {
          _allTables = tables
              .map<Map<String, dynamic>>((table) => {
                    '_id': table['_id'] ?? '',
        ...
        print('❌ Failed to load tables from popular API');
      }
    } catch (e) {
      print('❌ Critical error loading all tables: $e');
    } finally {
      setState(() {
        _isLoadingAll = false;
      });
